...
Parser::Parser(const std::string &json_filename) : json_(), parent_path_("")
{
    ...
    if (filepath.extension() != ".json") {
        throw std::invalid_argument(Logger::Error("Could not load configuration file. Expected JSON format (*.json)"));
    }
    ...
    if (file_stream.is_open()) {
        ...
        std::string error_msg = "Could not parse JSON input file. Check file path.";
...
void Parser::ResolveFilePath(std::string &filename) const
{
    auto filepath = std::filesystem::path{filename};
    auto filepath_string = std::string{filepath.parent_path()};
    if (!filepath.has_root_path() || filepath_string.size() == 1) {
        if (filename[0] == '/' || filename[0] == '\\') {
            filename = this->ParentPath() + filename;
        } else {
            filename = this->ParentPath() + "/" + filename;
        }
    }
